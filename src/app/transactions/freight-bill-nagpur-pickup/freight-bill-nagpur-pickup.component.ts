...
@Component({
  selector: 'app-freight-bill-nagpur-pickup',
  standalone: false,
  templateUrl: './freight-bill-nagpur-pickup.component.html',
  styleUrl: './freight-bill-nagpur-pickup.component.scss'
})
export class FreightBillNagpurPickupComponent {
  ...
  checkFreightBillExist(billNo: any) {
    this.apiService.getNagpurPickupSavedFreightByBillNo(billNo).subscribe(res => {
      if (res) {
  ...
  deleteNagpurPickupFreightBill() {
    let freightId = this.form.get('freightBillReportId')?.value;
    if (freightId) {
      this.apiService.deleteNagpurPickupFreightBill(freightId).subscribe(res => {
        if (res) {
          this.clearField();
          $.toast({
            heading: 'Bill Removed!',
            text: 'You have deleted the nagpur-pickup bill information!!',
            showHideTransition: 'fade',
            icon: 'info',
            position: 'top-center',
            bgColor: '#1e6421',
  ...
  saveNagpurPickupFreight() {
    ...
    this.apiService.saveNagpurPickupFreight(formObj).subscribe(res => {
      if (res) {
        this.clearField();
        ...
        $.toast({
          heading: 'Nagpur Pickup freight bill has been submitted!',
          text: 'You have submitted the Nagpur Pickup freight bill. Please contact to respective authority member for approval.',
          showHideTransition: 'fade',
          icon: 'info',
          position: 'top-center',
          bgColor: '#257b26',
  ...
  printFreight() {
    ...
    if (printWindow) {
      ...
      <head>
          ...
          <title>Nagpur Pickup Freight Bill (${this.billingCommonData.billNo})</title>
    ...
    if (printWindow) {
      ...
      setTimeout(() => {
        html2canvas(printWindow.document.body, {scale: 2}).then(canvas => {
          ...
          pdf.save('nagpur-pickup-freight-bill-' + this.billingCommonData.billNo + '.pdf');
          printWindow.close();
        });
      }, 1000);
  ...
  exportToExcel() {
    ...
    // Table headers
    let headers = [
      "LR No.", "Lorry Date", "Unloading Date", "Vehicle No", "Vendor Name",
      "Weight/Vehicle", "Rate/Kg", "Freight", "Loading Charges", "Plywood Charges",
      "Collection Charges", "ST Charges", "Total Bill"
    ];
    wsData.push(headers);
    ...
    this.billingData.forEach(l => {
      let row = [
        ...
        l.vehicleNo,
        l.vendorName,
        l.vehicleType || this.getDisplayTotalWeight(l.lrNo),
        2.66,
        this.getDisplayTotalFreight(l.lrNo),
        this.getDisplayTotalLoadingCharges(l.lrNo),
        this.getDisplayTotalPlywoodCharges(l.lrNo),
        this.getDisplayTotalCollectionCharges(l.lrNo),
        this.getDisplaySTCharges(l.lrNo) || 0,
        this.getTotalBillValue(l.lrNo).toFixed(2)
      ];
      wsData.push(row);
    });
    ...
    let footer = [
      "", "", "", "", "Total",
      showWeight ? this.getTotalWeight().toFixed(2) : "",
      showVehicleType ? "0.00" : "",
